...
export default function reducer(state={
    ...
    switch (action.type) {
      ...
      case events.SESSION_ADD_COUNTRIES: {
        let countries = new Set(state.selected_countries)
      ...
      case events.SESSION_ADD_STATES: {
        let states = new Set(state.selected_states)
      ...
      case events.SESSION_ADD_PROVINCES: {
        let provinces = new Set(state.selected_provinces)
      ...
      case events.SESSION_ADD_AIRPORT: {
        let airports = new Set(state.selected_airports)
      ...
      case events.SESSION_ADD_NAVAID: {
        let navaids = new Set(state.selected_navaids)
      ...
      case events.SESSION_ADD_MAP: {
        let map = state.selected_charts | action.payload
        return {
          ...state,
          selected_charts: map,
        }
      ...
      case events.SESSION_RESET_MAPS: {
        let map = state.map_types.filter(mt => mt.selected_by_default).reduce((acc, val) => acc|val.code, 0)
        return {
          ...state,
          selected_charts: map,
        }
      ...
      case events.SESSION_ADD_NOTE: {
        let notes = new Set(state.selected_notes)
